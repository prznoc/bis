...
def check_file(filepath):
    ...
    for i in frames.keys():
        models[i] = train_model(frames[i][['f', 'wday']].copy())
    ...
    j = 0
    for i in frames.keys():
        ypred0 = np.multiply(predict_flow(models[i], frames[i][['f', 'wday']].copy()), maxes[i])
    ...
    for ip in frames.keys():
        ent[ip] = (ApEn(np.multiply(frames[ip]['f'].values, 1), m, r))
    ...
    for i in frames.keys():
        et0 = entropy_trend(frames[i]['f'], days)
        if size < 2:
